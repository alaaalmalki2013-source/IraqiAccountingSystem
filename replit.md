# نظام المحاسبة العراقي - Iraqi Accounting System

## نظرة عامة

نظام محاسبة عراقي متكامل مصمم لإدارة الإيرادات والمصروفات والمخزون والموظفين والرواتب. يتميز النظام بواجهة عربية كاملة RTL مع تصميم احترافي ودعم للوضع الداكن/الفاتح.

## المميزات الرئيسية

### 1. **لوحة التحكم (Dashboard)**
- عرض الإحصائيات المالية الشاملة
- رسوم بيانية تفاعلية للإيرادات والمصروفات
- تنبيهات أعياد الميلاد (قبل يومين حتى نهاية يوم عيد الميلاد)
- ملخص الحالة المالية الحالية

### 2. **الشكل العام والتصميم**
- **Dark/Light Mode:** تبديل بين الوضع الداكن والفاتح مع حفظ التفضيل لكل مستخدم
- **Sidebar Collapse:** قائمة جانبية قابلة للطي/التوسيع مع حفظ الحالة
- **RTL Support:** دعم كامل للغة العربية من اليمين لليسار
- **Responsive Design:** تصميم متجاوب يعمل على جميع الأجهزة

### 3. **إدارة الإيرادات (Revenues)**
- إضافة وتعديل وحذف الإيرادات
- تصنيف الإيرادات حسب الفئات
- فلترة متقدمة حسب التاريخ والفئة
- بحث شامل يدعم الأرقام العربية
- طباعة وتصدير التقارير إلى CSV

### 4. **إدارة المصروفات (Expenses)**
- تسجيل المصروفات مع تفاصيل الموردين والمندوبين
- ربط المصروفات بالمشتريات المخزنية
- نظام فواتير متقدم
- طباعة سندات الصرف (80mm و A4)

### 5. **إدارة الموظفين (Employees)**
- قاعدة بيانات كاملة للموظفين
- تتبع أعياد الميلاد
- إدارة الرواتب الأساسية
- تخزين المستندات الشخصية

### 6. **نظام السلف (Advances)**
- إدارة السلف المالية للموظفين
- **فئات السلف:** سلفة شخصية، سلفة طارئة، سلفة علاجية، سلفة عائلية، أخرى
- تتبع المدفوعات والمستحقات
- طباعة سندات السلف
- فلترة حسب الفئة

### 7. **المبالغ المعلقة (Suspended)**
- تتبع المدفوعات المعلقة
- إدارة الذمم والديون

### 8. **الإدخال المخزني (Inventory Entry)** ⭐ محدّث
- إدخال فواتير المشتريات
- **Autocomplete ذكي:** اقتراحات تلقائية لأسماء المواد من المخزون
- **تعديل المواد:** إمكانية تعديل وحذف المواد بعد إضافتها للفاتورة
- **حقل الكمية RTL:** كتابة الكمية من اليمين
- اعتماد الفواتير (كاش أو آجل)
- إلغاء الفواتير مع تسجيل السبب
- تتبع حالات الفواتير (معلقة، معتمدة كاش، معتمدة آجل، ملغاة)

### 9. **إدارة المخزون (Inventory)**
- قاعدة بيانات كاملة للمواد
- نظام باركود متكامل
- تتبع سجل المشتريات لكل مادة
- صرف المواد من المخزون
- تتبع المخزون الحالي

### 10. **نظام الرواتب (Payroll)** ⭐ جديد
- **جدول الرواتب الشامل:** عرض جميع الموظفين مع تفاصيل الراتب
- **الأعمدة:** الاسم، الراتب الأساسي، المكافآت، الخصومات، الغياب، الأوفرتايم، السلف، الراتب الصافي، الحالة، الإجراءات
- **إضافة تعديلات:** مكافآت، خصومات، غياب، أوفرتايم
- **سجل السلف:** عرض تلقائي للسلف المستحقة من قسم السلف
- **حساب تلقائي:** حساب الراتب الصافي تلقائياً
- **طباعة كشف راتب 80mm:** تنسيق احترافي للطابعة الحرارية
- **دفع الراتب:** تتبع حالة الدفع (مستلم/غير مستلم)
- **فلترة:** الكل، مدفوعة، غير مدفوعة
- **اختيار الشهر والسنة:** عرض الرواتب لشهر معين

### 11. **الإعدادات (Settings)**
- إدارة الفئات (إيرادات ومصروفات)
- **إدارة فئات السلف** ⭐ جديد
- إدارة الأقسام والمناصب الوظيفية
- إدارة الموردين والمندوبين
- نظام صلاحيات المستخدمين
- إعدادات الشركة (الاسم، الشعار)

### 12. **المساعد الذكي AI Chatbot** ⭐ جديد
- **دعم ذكي بالذكاء الاصطناعي:** مساعد محادثة يعمل بـ GPT-5
- **إجابات فورية:** يجيب على أسئلة المستخدمين حول كيفية استخدام النظام
- **واجهة عائمة:** نافذة محادثة عائمة في أسفل الصفحة
- **تصميم عربي RTL:** دعم كامل للغة العربية
- **دعم Dark Mode:** يتكامل مع وضع النظام الداكن/الفاتح
- **سياق النظام:** يفهم جميع أقسام ومميزات النظام
- **سجل المحادثة:** يحفظ سياق المحادثة للإجابات الأفضل

## التقنيات المستخدمة

### Frontend
- **React** - مكتبة واجهة المستخدم
- **Tailwind CSS** - تصميم الواجهة
- **Lucide React** - الأيقونات
- **LocalStorage** - تخزين البيانات محلياً

### Backend
- **Express.js** - خادم الويب
- **TypeScript** - لغة البرمجة
- **OpenAI GPT-5** - المساعد الذكي (عبر Replit AI Integrations)
- **REST API** - نقطة نهاية /api/chat للمساعد الذكي

## البنية المعمارية

```
client/
  ├── src/
  │   ├── pages/
  │   │   └── AccountingApp.tsx  # المكون الرئيسي (4700+ سطر) + مكون AIChatbot
  │   ├── App.tsx                # نقطة الدخول
  │   ├── index.css              # التصميم الأساسي
  │   └── ...
  ├── index.html                 # HTML الرئيسي مع دعم العربية
server/
  ├── index.ts                   # خادم Express
  ├── routes.ts                  # API endpoints (chat endpoint)
  ├── lib/
  │   └── openai.ts              # OpenAI client configuration
  └── ...
```

## المميزات التقنية

### 1. **البحث الذكي**
- دعم الأرقام العربية والإنجليزية
- بحث شامل في جميع الحقول
- تمييز النتائج المطابقة
- Autocomplete في الإدخال المخزني

### 2. **الطباعة**
- طباعة سندات الصرف (80mm للحرارية، A4 للعادية)
- طباعة كشوف الرواتب 80mm
- طباعة التقارير الشاملة
- معاينة قبل الطباعة

### 3. **التصدير**
- تصدير البيانات إلى CSV
- دعم الترميز العربي (UTF-8 with BOM)

### 4. **الواجهة RTL**
- تصميم كامل من اليمين لليسار
- خطوط عربية احترافية (Cairo, IBM Plex Arabic, Tajawal)
- ألوان مخصصة للفئات
- دعم Dark/Light Mode في جميع الأقسام

### 5. **نظام الصلاحيات**
- صلاحيات مفصلة لكل قسم
- دعم مستخدمين متعددين
- التحكم في الرؤية والإضافة والتعديل والحذف

## كيفية الاستخدام

### التشغيل المحلي

```bash
npm install
npm run dev
```

التطبيق سيعمل على: `http://localhost:5000`

### البيانات الافتراضية

- المستخدم الافتراضي: `المدير العام`
- كلمة المرور: `password`
- النظام يبدأ بدون بيانات تجريبية (production-ready)

## الحالة الحالية

- ✅ جميع المكونات الأساسية تعمل
- ✅ التخزين المحلي (localStorage) نشط
- ✅ الواجهة العربية الكاملة
- ✅ نظام الطباعة والتصدير
- ✅ البحث المتقدم والفلترة
- ✅ الإحصائيات والرسوم البيانية
- ✅ Dark/Light Mode
- ✅ Sidebar Collapse/Expand
- ✅ نظام الرواتب الكامل
- ✅ فئات السلف
- ✅ Inventory Autocomplete
- ✅ المساعد الذكي AI Chatbot
- ✅ الوضع الداكن الشامل مع dark: variants
- ✅ زر الوضع الداكن في أسفل القائمة (أيقونة فقط)

## التطوير المستقبلي

1. **قاعدة بيانات PostgreSQL** - لحفظ البيانات بشكل دائم
2. **نظام المصادقة** - Replit Auth لتسجيل الدخول الآمن
3. **الإشعارات** - تنبيهات للعمليات الهامة
4. **تطبيق الموبايل** - للوصول السريع
5. **النسخ الاحتياطي التلقائي** - لحماية البيانات
6. **تقارير متقدمة** - تقارير مالية شاملة
7. **تكامل مع الأنظمة الخارجية** - API للتكامل

## الملاحظات المهمة

- البيانات محفوظة في `localStorage` تحت المفتاح: `IRAQI_ACCOUNTING_DATA_V3_LOCAL`
- يمكن تصدير/استيراد البيانات عبر نسخ localStorage
- النظام مصمم للعمل بدون اتصال بالإنترنت
- تفضيلات Dark Mode و Sidebar Collapse محفوظة لكل مستخدم
- أعياد الميلاد تظهر قبل يومين من عيد الميلاد حتى نهاية اليوم
- نظام الرواتب متكامل مع قسم السلف

## التحديثات الأخيرة (2025-10-19)

### الإصدار 3.2 - المساعد الذكي AI + تحسينات Dark Mode

1. **المساعد الذكي AI Chatbot (جديد):**
   - مساعد محادثة ذكي يعمل بـ GPT-5-mini
   - واجهة عائمة احترافية في أسفل الصفحة
   - إجابات فورية عن أسئلة المستخدمين
   - فهم كامل لجميع أقسام ومميزات النظام
   - سجل محادثة يحفظ السياق
   - تصميم RTL كامل مع دعم Dark Mode
   - API endpoint: POST /api/chat

2. **تحسينات الوضع الداكن:**
   - إضافة dark: variants شاملة لجميع العناصر
   - تحسين ألوان النصوص والخلفيات في الوضع الداكن
   - إصلاح تباين الألوان للبطاقات والجداول
   - نقل زر الوضع الداكن لأسفل القائمة (أيقونة فقط)
   - تحسين تجربة المستخدم في الوضع الداكن

3. **الشكل العام:**
   - إضافة Dark/Light Mode مع حفظ التفضيل
   - قائمة جانبية قابلة للطي
   - تحديث birthday reminders (قبل يومين)
   - إصلاح المجاميع في Dashboard

4. **الإدخال المخزني:**
   - Autocomplete ذكي لأسماء المواد
   - إمكانية تعديل وحذف المواد بعد الإضافة
   - حقل الكمية RTL

5. **السلف:**
   - إضافة فئات السلف (شخصية، طارئة، علاجية، عائلية، أخرى)
   - فلترة حسب الفئة
   - إدارة الفئات في الإعدادات

6. **الموظفين:**
   - إزالة حقل أيام الغياب (تم نقله للرواتب)
   - تبسيط الواجهة

7. **نظام الرواتب:**
   - صفحة كاملة لإدارة الرواتب
   - جدول شامل مع 10 أعمدة
   - إضافة تعديلات (مكافآت، خصومات، غياب، أوفرتايم)
   - عرض تلقائي للسلف
   - حساب الراتب الصافي تلقائياً
   - طباعة كشف راتب 80mm
   - دفع الراتب وتتبع الحالة
   - فلترة (الكل، مدفوعة، غير مدفوعة)

## الدعم والمساعدة

للحصول على المساعدة أو الإبلاغ عن مشاكل:
- افتح الكونسول (F12) لرؤية أي أخطاء
- تحقق من سجل الشبكة (Network)
- راجع بيانات localStorage

## ملاحظات تقنية إضافية

- **المساعد الذكي:** يستخدم Replit AI Integrations (لا يتطلب API key خاص)
- **النموذج المستخدم:** GPT-5-mini (أحدث نموذج من OpenAI)
- **التكلفة:** تُحسب من أرصدة Replit الخاصة بك
- **نقطة API:** POST /api/chat مع body: { message, conversationHistory }

---

آخر تحديث: 2025-10-19
الإصدار: 3.2 (AI Chatbot Support + Enhanced Dark Mode)

